...
if [[ -z $NITSDECTHREADS ]]; then NITSDECTHREADS=2; fi
if [[ -z $NMFTDECTHREADS ]]; then NMFTDECTHREADS=2; fi

if [[ -z $NMFTTHREADS ]]; then NMFTTHREADS=2; fi

if [[ -z $SVERTEX_THREADS ]]; then SVERTEX_THREADS=$(( SYNCMODE == 1 ? 1 : 2 )); fi
...
if [[ $SYNCMODE == 1 ]]; then NTRDTRKTHREADS=1; fi

if [[ -z $NGPURECOTHREADS ]]; then NGPURECOTHREADS=-1; fi # -1 = auto-detect
...
if [[ ! -z $OPTIMIZED_PARALLEL_ASYNC ]]; then
  ...
  if [[ $OPTIMIZED_PARALLEL_ASYNC == "pp_8cpu" ]]; then
    NGPURECOTHREADS=5
    if [[ -z $TIMEFRAME_RATE_LIMIT ]]; then TIMEFRAME_RATE_LIMIT=3; fi
  elif [[ $OPTIMIZED_PARALLEL_ASYNC == "pp_1gpu" ]]; then
    if [[ -z $TIMEFRAME_RATE_LIMIT ]]; then TIMEFRAME_RATE_LIMIT=8; fi
    if [[ -z $SHMSIZE ]]; then SHMSIZE=24000000000; fi
    N_TOFMATCH=2
    N_MCHCL=3
    N_TPCENTDEC=3
    N_TPCITS=3
    ...
    if [[ -z $TIMEFRAME_RATE_LIMIT ]]; then TIMEFRAME_RATE_LIMIT=32; fi
    if [[ -z $SHMSIZE ]]; then SHMSIZE=80000000000; fi
    NGPURECOTHREADS=8
    NTRDTRKTHREADS=2
    NGPUS=4
    N_TPCTRK=4
    ...
    if [[ -z $TIMEFRAME_RATE_LIMIT ]]; then TIMEFRAME_RATE_LIMIT=20; fi
    if [[ -z $SHMSIZE ]]; then SHMSIZE=128000000000; fi # SHM_LIMIT 3/4
    NGPURECOTHREADS=8
    NTRDTRKTHREADS=4
    NGPUS=4
    N_TPCTRK=4
...
if [[ -z $EVE_NTH_EVENT ]]; then
  if [[ $BEAMTYPE == "PbPb" ]]; then
    if [[ -z $EVE_NTH_EVENT ]]; then EVE_NTH_EVENT=2; fi
  elif [[ "0$HIGH_RATE_PP" == "01" ]]; then
    if [[ -z $EVE_NTH_EVENT ]]; then EVE_NTH_EVENT=10; fi
  elif [[ $BEAMTYPE == "pp" && "0$ED_VERTEX_MODE" == "01" ]]; then
    if [[ -z $EVE_NTH_EVENT ]]; then EVE_NTH_EVENT=$((4 * 250 / RECO_NUM_NODES_WORKFLOW_CMP)); fi
  fi
fi

if [[ "0$HIGH_RATE_PP" == "01" ]]; then
  if [[ -z $CUT_RANDOM_FRACTION_ITS ]]; then CUT_RANDOM_FRACTION_ITS=0.97; fi
else
  if [[ -z $CUT_RANDOM_FRACTION_ITS ]]; then CUT_RANDOM_FRACTION_ITS=0.95; fi
fi
if [[ -z $CUT_RANDOM_FRACTION_MCH && $RUNTYPE != "COSMICS" ]]; then CUT_RANDOM_FRACTION_MCH=0.7; fi
