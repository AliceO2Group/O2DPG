int External()
{
	...
	if (nleptonsInAcceptance > 1) nLeptonPairsInAcceptance++;
    }
    ...
    if (nLeptonPairsInAcceptance < nEvents) {
        std::cerr << "The number of lepton pairs in acceptance should be at least the same as the total number of events\n";
        return 1;
    }
    return 0;
}
