...
// that alternates between 2 gun generators. This is the non-triggered version, without hooks, and would work with any PYTHIA cfg
...
class GeneratorPythia8GapGenJEMB : public o2::eventgen::GeneratorPythia8
{
public:
  /// default constructor
  GeneratorPythia8GapGenJEMB(int inputTriggerRatio = 5,std::string pathMB = "",std::string pathSignal = "") {
    ...
    // Here we could loada hook, but we want to run MB for this production
  ...
  ///  Destructor
  ~GeneratorPythia8GapGenJEMB() = default;
...
FairGenerator* getGeneratorPythia8GapGenJEMB(int inputTriggerRatio = 5, std::string pathMB = "",std::string pathSignal = "") {
  auto myGen = new o2::eventgen::GeneratorPythia8GapGenJEMB(inputTriggerRatio, pathMB, pathSignal);
  auto seed = (gRandom->TRandom::GetSeed() % 900000000);
