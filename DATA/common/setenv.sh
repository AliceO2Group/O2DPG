...
if [ $EPNSYNCMODE == 0 ]; then
  ...
  if [ -z "${WORKFLOW_DETECTORS_FLP_PROCESSING+x}" ]; then export WORKFLOW_DETECTORS_FLP_PROCESSING="TOF,FT0,FV0,FDD"; fi # Current default in sync processing is that FLP processing is only enabled for TOF
