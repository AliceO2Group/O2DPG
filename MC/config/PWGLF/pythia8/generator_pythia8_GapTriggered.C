...
public:
  ...
  GeneratorPythia8GunPbPb(int input_pdg){
    ...
    lGeneratedEvents=0;
    lNonTriggeredToTriggeredRatio=4; //4:1
...
//      3  p2           6.54815e+00   3.09274e-02   3.09274e-02   4.20673e-02
    ...
    // Read settings from external file. This part could be improved further!
    pythiaObject.readFile("${O2DPG_ROOT}/MC/config/PWGLF/pythia8/pythia8_inel_randomseed.cfg");
...
protected:
  ...
  Bool_t generateEvent() override {
    ...
    //+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+
    // Generate base event, please
    Bool_t lPythiaOK = kFALSE;
    int lInverseTriggerRatio = lNonTriggeredToTriggeredRatio+1;
    while (!lPythiaOK){
      Bool_t lGenerationOK = kFALSE;
      if(!lGenerationOK) lGenerationOK = pythiaObject.next();
      //Check if event interesting
      for ( Long_t j=0; j < pythiaObject.event.size(); j++ ) {
        Int_t pypid = pythiaObject.event[j].id();
        Float_t pyeta = pythiaObject.event[j].eta();
        if( pypid == pdg && genminEta<pyeta && pyeta<genmaxEta){
          lPythiaOK = kTRUE;
          break;
        }
      }
      if( lGeneratedEvents % lInverseTriggerRatio != 0 ) lPythiaOK = kTRUE;
    }
    //+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+
    // use event - FIXME: save history, a la ImportParticles
...
private:
  ...
  int lNonTriggeredToTriggeredRatio;
