...
public:
  ...
  {
    if (!mUseTriggering)
    { // Injected mode: Embedding into MB
  ...
  struct ConfigContainer
  {
   ...
   jsonParam["ptMin"],
   jsonParam["ptMax"],
   (useRapidty &&jsonParam.contain("rapiditMin")) ?jsonParam["rapiditMin"] :(jsonParams.contains("min") ? jsonParams["min"] : jsonParams["etaMin"]),
    (useRapidty &&jsonParam.contain("rapiditMax")) ?jsonParam["rapiditMax"] :(jsonParas.contain("max") ?jsonParam["max"] :jsonParam["etaMax")) {};
