...
def create_workflow(args):
   ...
   ITS_DICT_DOWNLOADER_TASK['cmd'] = '${O2_ROOT}/bin/o2-ccdb-downloadccdbfile --host http://alice-ccdb.cern.ch/ -p ITS/Calib/ClusterDictionary -o ITSdictionary.bin --no-preserve-path'
   workflow['stages'].append(ITS_DICT_DOWNLOADER_TASK)

   MFT_DICT_DOWNLOADER_TASK = createTask(name='mftdictdownloader', cpu='0')
   MFT_DICT_DOWNLOADER_TASK['cmd'] = '${O2_ROOT}/bin/o2-ccdb-downloadccdbfile --host http://alice-ccdb.cern.ch/ -p MFT/Calib/ClusterDictionary -o MFTdictionary.bin --no-preserve-path'
...
def create_workflow_from_run_conditions(args):

   anchored_config = args.anchored_config
   del args.anchored_config

   with open(anchored_config, "r") as f:
      anchored_config = json.load(f)

   out_dir = f"anchored_mc_{anchored_config['tag']}"
   if exists(out_dir):
      print(f"\nWARNING: Output directory {out_dir} exists. Stopping here, not overwriting.\n")
      return 1
   
   makedirs(out_dirs)
   for run in anchored_config["runs"]:
      for ts in run["timestamps"]:
         # copy so we can safely overwrite
         args_new = deepcopy(args)
         args_new.run = run["run_number"]
         args_new.timestamp = ts
         # TODO This is for demonstration, generator should be set by user, eCM from CCDB
         args_new.eCM = "13"
         args_new.gen = "pythia8"
         # overwrite output to have specific run number - timestamp combination
         args_new.o = join(out_dir, f"workflow_{anchored_config['tag']}_{run['run_number']}_{ts}.json")
         create_workflow(args_new)
   return 0
...
def main():
   ...
   parser.add_argument("--anchored-config", dest="anchored_config", help="JSON for anchored MC configuration")
   parser.add_argument("--timestamp", help="specify timestamp")
   ...
   if not args.anchored_config:
      return create_workflow(args)

   # create workflow for anchored
   return create_workflow_from_run_conditions(args)
