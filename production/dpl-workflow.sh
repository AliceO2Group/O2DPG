...
if [ $CTFINPUT == 1 ]; then
  ...
  PROXY_CHANNEL="name=readout-proxy,type=pull,method=connect,address=ipc://@$INRAWCHANNAME,transport=shmem"
  [[ $EPNMODE == 1 ]] && PROXY_CHANNEL+=",rateLogging=0"
  [[ $EPNMODE == 1 ]] && PROXY_CHANNEL+=",rateLogging=1"
