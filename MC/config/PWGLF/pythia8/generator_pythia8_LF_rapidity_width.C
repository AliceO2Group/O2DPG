...
class GeneratorPythia8LFRapidity : public o2::eventgen::GeneratorPythia8
{
 public:
  ...
  struct ConfigContainer {
                                                     ...
                                                     mMax{max}
    {
      ...
      if (mMass <= 0) {
        LOG(warning) << "Could not find mass for mPdg " << mPdg << " in TDatabasePDG; will try Pythia particle data at injection time";
      }
  ...
  {
    const double pythiaMass = mPythiaGun.particleData.m0(cfg.mPdg);
    const double mass = mPythiaGun.particleData.mWidth(cfg.mPdg) > 0.
                          ? mPythiaGun.particleData.mSel(cfg.mPdg)
                          : pythiaMass > 0.
                              ? pythiaMass
                              : cfg.mMass;
    if (mass <= 0.) {
      LOG(fatal) << "Could not find mass for mPdg " << cfg.mPdg;
    }
    return mass;
  }
 ...
 private:
  ...
  static bool readParticleDataLines(Pythia8::Pythia& pythia, const std::string& config, const char* target)
  {
    ...
    if (!input.is_open()) {
      LOG(fatal) << "Could not open particle data configuration " << config << " for " << target;
      return false;
    }
    std::string line;
    while (std::getline(input, line)) {
      ...
      LOG(info) << "Reading particle data for " << target << ": " << command;
      if (!pythia.readString(command)) {
        LOG(fatal) << "Failed to read particle data line for " << target << ": " << command;
        return false;
      }
    }
    return true;
