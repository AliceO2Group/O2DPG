int External()
{
    ...
    int nQuarks{}, nSignals{}, nSignalGoodDecay{}, nEventsWithJPsiInY{};
    auto nEvents = tree->GetEntries();

    for (int i = 0; i < nEvents; i++)
    {
        tree->GetEntry(i);
        bool isJPsiInYRange{false};
    ...
    float fracOfEventsWithJPsiInY = float(nEventsWithJPsiInY) / nEvents;
    std::cout <<"fraction of events with J/psi in y acceptance: " << fracOfEventsWithJPsiInY << "\n";
