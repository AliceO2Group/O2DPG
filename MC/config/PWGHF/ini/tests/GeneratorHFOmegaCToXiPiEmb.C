int External()
{
    ...
    if ((nDauPairs != nPiFromOmegacToBeDone) || (nPiFromOmegacToBeDone != nPiFromCascToBeDone) || (nPiFromCascToBeDone != nPiFromLamToBeDone) || (nPiFromLamToBeDone != nPFromLamToBeDone))
    {
        std::cerr << "The number of daughter pairs should be the same as the number of pi<-omegac, of pi<-xi, of pi<-lambda and of p<-lambda which should be transported.\n";
        return 1;
    }
