...
#
#Running: TIME="#walltime %e\n#systime %S\n#usertime %U\n#maxmem %M\n#CPU %P" /usr/bin/time --output=sgnsim_1.log_time ./sgnsim_1.log_tmp.sh
#[INFO] This is o2-sim version 1.2.0 (9a4248308)
#[INFO] Built by ALIBUILD:1.14.4, ALIDIST-REV:f51b1b17e33a1922a1804449cd1759a74e0c7bf6 on OS:Linux-5.4.0-135-generic
#[INFO] BINDING TO ADDRESS ipc:///tmp/o2sim-notifications-4297 type pub
#[FATAL] Module ZDC not constructed and cannot be used for readout (make sure it is contained in -m option).
#For later analysis we write a core dump to core_dump_4297
#
...
### async_pass.sh
export O2DPGPATH="${ALIEN_JDL_LPMPRODUCTIONTAG}"
[[ -n "$ALIEN_JDL_O2DPGPATH" ]] && O2DPGPATH="$ALIEN_JDL_O2DPGPATH"

export O2DPGPASSPATH="${ALIEN_JDL_LPMPASSNAME}"
[[ -n "$ALIEN_JDL_O2DPGPASSPATH" ]] && O2DPGPASSPATH="$ALIEN_JDL_O2DPGPASSPATH"

( [[ $ALIEN_JDL_LPMPRODUCTIONTAG == MAY ]] || [[ $ALIEN_JDL_LPMPRODUCTIONTAG == LHC22c ]] ) && O2DPGPATH="MayJunePilotBeam"
DPGRECO=$O2DPG_ROOT/DATA/production/configurations/$ALIEN_JDL_LPMANCHORYEAR/$O2DPGPATH/$O2DPGPASSPATH/async_pass.sh
...
echo "[INFO alien_async_pass.sh] Setting up O2DPGPATH to ${O2DPGPATH}"
...
# for reco, this is needed just for the async reco QC, not for MC
# default is fine for apass3 of LHC22f...m, but will be provided via JDL (ie for later periods 22o...t)
[[ ! -f commonInput.tgz ]] && alien.py cp /alice/cern.ch/user/a/alidaq/LHC22f/apass3/update69/commonInput.tgz file:.
...
# THIS NEEDS TO COME FROM OUTSIDE
remainingargs="-eCM 13600 -col pp -gen pythia8 -proc cdiff -ns ${NSIGEVENTS} \
