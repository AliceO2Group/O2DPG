...
#include "Generator_pythia8_HFLepton_pp.C"
...
{
    enum GenType : int {
        Charm = 0,            // --> GeneratorPythia8CharmLepton: charm enriched
        Beauty,               // --> GeneratorPythia8BeautyForcedDecays: beauty enriched with forced decays
        BeautyNoForcedDecays, // --> GeneratorPythia8BeautyNoForcedDecays: beauty enriched no forced decays
        NGenType
    };
}

class GeneratorPythia8EmbedHFLepton : public o2::eventgen::GeneratorPythia8
{
public:
    ...
    ~GeneratorPythia8EmbedHFLepton() {
        ...
        if (mGeneratorEvHFLepton) {
            delete mGeneratorEvHFLepton;
            mGeneratorEvHFLepton = nullptr;
        }
    }
    ...
    void setupGeneratorEvHFLepton(int genType, int inputExternalID, int pdgLepton, float yMinQ = -1.5, float yMaxQ = 1.5, float yMinL = -1, float yMaxL = 1) {
      mGeneratorEvHFLepton = nullptr;
        switch (genType)
        {
        case hf_generators::Charm:
            LOG(info) << "********** [GeneratorPythia8EmbedHFLepton] configuring GeneratorPythia8CharmLepton **********";
            LOG(info) << "**********                           Default number of HF signal events to be merged (updated by notifyEmbedding): " << mNumSigEvs;
            mGeneratorEvHFLepton = dynamic_cast<GeneratorPythia8HFLeptonpp*>(GeneratorPythia8CharmLepton(inputExternalID, pdgLepton, yMinQ, yMaxQ, yMinL, yMaxL));
            break;
        case hf_generators::Beauty:
            LOG(info) << "********** [GeneratorPythia8EmbedHFLepton] configuring GeneratorPythia8BeautyForcedDecays **********";
            LOG(info) << "**********                           Default number of HF signal events to be merged (updated by notifyEmbedding): " << mNumSigEvs;
            mGeneratorEvHFLepton = dynamic_cast<GeneratorPythia8HFLeptonpp*>(GeneratorPythia8BeautyForcedDecays(inputExternalID, pdgLepton, yMinQ, yMaxQ, yMinL, yMaxL));
            break;
        case hf_generators::BeautyNoForcedDecays:
            LOG(info) << "********** [GeneratorPythia8EmbedHFLepton] configuring GeneratorPythia8BeautyNoForcedDecays **********";
            LOG(info) << "**********                           Default number of HF signal events to be merged (updated by notifyEmbedding): " << mNumSigEvs;
            mGeneratorEvHFLepton = dynamic_cast<GeneratorPythia8HFLeptonpp*>(GeneratorPythia8BeautyNoForcedDecays(inputExternalID, pdgLepton, yMinQ, yMaxQ, yMinL, yMaxL));
            break;
        default:
            LOG(fatal) << "********** [GeneratorPythia8EmbedHFLepton] bad configuration, fix it! **********";
            break;
        }
        mGeneratorEvHFLepton->Init();
    }
...
Bool_t importParticles() override
{
  ...
  while(nEvsHF < mNumSigEvs) {
      ...
      bool genOk = false;
      while(!genOk) {
        genOk = (mGeneratorEvHFLepton->generateEvent() && mGeneratorEvHFLepton->importParticles() /*copy particles from mGeneratorEvHF.mPythia.event to mGeneratorEvHF.mParticles*/ );
      }
      ...
      auto particlesHfEvent = mGeneratorEvHFLepton->getParticles();
      ...
      nEvsHF++;
      //mGeneratedEvents++;
      mGeneratorEvHFLepton->clearParticles();
  }
...
private:
    ...
    Generator* mGeneratorEvHFLepton; // to generate HF signal events
...
// Charm enriched
FairGenerator * GeneratorPythia8EmbedHFLeptonCharm(int inputExternalID, int pdgLepton, float yMinQ = -1.5, float yMaxQ = 1.5, float yMinL = -1, float yMaxL = 1)
{
    auto myGen = new GeneratorPythia8EmbedHFLepton();

    /// setup the internal generator for HF events
    myGen->setupGeneratorEvHFLepton(hf_generators::Charm, inputExternalID, pdgLepton, yMinQ = -1.5, yMaxQ = 1.5, yMinL = -1, yMaxL = 1);

    return myGen;
}

// Beauty enriched
FairGenerator * GeneratorPythia8EmbedHFLeptonBeauty(int inputExternalID, int pdgLepton, float yMinQ = -1.5, float yMaxQ = 1.5, float yMinL = -1, float yMaxL = 1)
{
    auto myGen = new GeneratorPythia8EmbedHFLepton();

    /// setup the internal generator for HF events
    myGen->setupGeneratorEvHFLepton(hf_generators::Beauty, inputExternalID, pdgLepton, yMinQ = -1.5, yMaxQ = 1.5, yMinL = -1, yMaxL = 1);

    return myGen;
}

// Charm and beauty enriched (with same ratio)
FairGenerator * GeneratorPythia8EmbedHFLeptonBeautyNoForcedDecays(int inputExternalID, int pdgLepton, float yMinQ = -1.5, float yMaxQ = 1.5, float yMinL = -1, float yMaxL = 1)
{
    auto myGen = new GeneratorPythia8EmbedHFLepton();

    /// setup the internal generator for HF events
    myGen->setupGeneratorEvHFLepton(hf_generators::BeautyNoForcedDecays, inputExternalID, pdgLepton, yMinQ = -1.5, yMaxQ = 1.5, yMinL = -1, yMaxL = 1);
